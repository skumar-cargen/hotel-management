...
class AlQusaisDubaiHotelsSeeder extends Seeder
{
    private int $counter = 600;

    public function run(): void
    {
        $this->command->info('Seeding Al Qusais Dubai Hotels domain...');
        ...
        $this->command->info('Al Qusais Dubai Hotels seeded successfully!');
    }
    ...
    private function seedDomain(): Domain
    {
        $domain = Domain::updateOrCreate(
            ['slug' => 'al-qusais-dubai-hotels'],
            [
                'name' => 'Al Qusais Dubai Hotels',
                'domain' => 'alqusaisdubaihotels.com',
                'is_active' => true,
                'is_primary' => false,
                'default_currency' => 'AED',
                'default_language' => 'en',
                'meta_title' => 'Al Qusais Dubai Hotels — Affordable Hotels in Al Qusais, Dubai',
                'meta_description' => 'Book affordable hotels in Al Qusais, Dubai. Comfortable stays near Dubai Airport, Al Nahda, and major shopping areas with excellent Metro access.',
                'meta_keywords' => 'al qusais hotels, budget hotels dubai, al nahda hotels, muhaisnah hotels, affordable dubai hotels, near dubai airport hotels',
                'phone' => '+971 4 267 3300',
                'email' => 'info@alqusaisdubaihotels.com',
                'address' => 'Al Qusais Industrial Area 1, Dubai, UAE',
            ]
        );
    ...
    private function seedHeroSlides(Domain $domain): void
    {
        ...
        $slides = [
            [
                'title' => 'Discover',
                'highlight' => 'Al Qusais\' Best Hotels',
                'subtitle' => 'Affordable comfort in one of Dubai\'s most connected neighbourhoods',
                'description' => 'Smart hotels with Metro access and proximity to Dubai Airport.',
                'keywords' => 'dubai,hotel,modern,affordable',
            ],
            [
                'title' => 'Stay Smart',
                'highlight' => 'Near Dubai Airport',
                'subtitle' => 'Just 15 minutes from DXB — perfect for early flights and layovers',
                'description' => 'Convenient, comfortable, and budget-friendly airport-area hotels.',
                'keywords' => 'hotel,room,clean,modern',
            ],
            [
                'title' => 'Experience',
                'highlight' => 'Real Dubai Living',
                'subtitle' => 'Local restaurants, traditional souks, and genuine Emirati hospitality',
                'description' => 'Discover Dubai beyond the tourist trail in authentic Al Qusais.',
                'keywords' => 'dubai,city,street,culture',
            ],
            [
                'title' => 'Value Meets',
                'highlight' => 'Comfort',
                'subtitle' => 'Spacious rooms, great service, and prices that make sense',
                'description' => 'Quality accommodation without the premium price tag.',
                'keywords' => 'hotel,interior,comfortable,clean',
            ],
        ];
    ...
    private function seedContentPages(Domain $domain): void
    {
        ...
        $domain->update([
            ...
            'about_us_meta_description' => "Learn about {$brandName} — your guide to affordable, well-connected hotels in Al Qusais, Dubai. Discover our mission, values, and commitment to value hospitality.",
    ...
    private function generateLogoPng(string $path, string $brandName): void
    {
        ...
        // Warm orange brand color
        $iconColor = imagecolorallocate($img, 210, 120, 40);
        $iconSize = 50;
        imagefilledrectangle($img, 10, 35, 10 + $iconSize, 35 + $iconSize, $iconColor);

        $white = imagecolorallocate($img, 255, 255, 255);
        $initial = 'AQ';
        imagestring($img, 5, 21, 52, $initial, $white);

        $textColor = imagecolorallocate($img, 30, 30, 50);
        imagestring($img, 5, 75, 42, 'Al Qusais Dubai', $textColor);
    ...
    private function generateFaviconPng(string $path, string $brandName): void
    {
        ...
        $bgColor = imagecolorallocate($img, 210, 120, 40);
        imagefilledrectangle($img, 4, 4, $size - 5, $size - 5, $bgColor);

        $white = imagecolorallocate($img, 255, 255, 255);
        $tw = imagefontwidth(5);
        $th = imagefontheight(5);
        imagestring($img, 5, ($size - $tw * 2) / 2, ($size - $th) / 2, 'AQ', $white);
...
<p>At <strong>{$brandName}</strong>, we specialise in connecting travellers with affordable, comfortable hotels in Al Qusais and the surrounding neighbourhoods of eastern Dubai. Whether you're catching an early flight from Dubai International Airport, visiting family, or exploring the city on a budget, we help you find the perfect place to stay.</p>

<h3>Our Mission</h3>
<p>Our mission is to make booking a quality hotel in Al Qusais simple, transparent, and affordable. We carefully select every property to ensure it meets high standards of cleanliness, comfort, and value — so you get the most out of your stay without breaking the bank.</p>

<h3>What Sets Us Apart</h3>
<ul>
    <li><strong>Budget-Friendly Specialists</strong> — Every hotel in our collection offers genuine value for money in one of Dubai's most accessible neighbourhoods.</li>
    <li><strong>Airport Proximity</strong> — Our hotels are just 10–15 minutes from Dubai International Airport, ideal for early flights and layovers.</li>
    <li><strong>Metro Connected</strong> — All our properties are near Metro stations, giving you easy access to the entire city.</li>
    <li><strong>Local Expertise</strong> — Our Dubai-based team knows Al Qusais inside and out, from the best local restaurants to hidden gems.</li>
    <li><strong>Flexible Booking</strong> — Free cancellation on most properties up to 48 hours before check-in.</li>
</ul>

<h3>Our Story</h3>
<p>{$brandName} was founded by a team of hospitality professionals who recognised that Al Qusais and its neighbouring areas offer some of the best value accommodation in Dubai. While the glitzy beachfront hotels get all the attention, savvy travellers know that eastern Dubai delivers comfort, convenience, and authenticity at a fraction of the price.</p>
<p>Today we help thousands of guests discover the real Dubai — a city of diverse neighbourhoods, local eateries, and warm hospitality that goes far beyond the tourist trail.</p>

<h3>Our Values</h3>
<ul>
    <li><strong>Transparency</strong> — No hidden fees, no misleading photos. What you see is what you get.</li>
    <li><strong>Value</strong> — We believe quality accommodation should be accessible to every traveller.</li>
    <li><strong>Guest-First</strong> — Every decision starts with: "Is this best for our guests?"</li>
    <li><strong>Authenticity</strong> — We celebrate the real Dubai and its diverse, vibrant communities.</li>
</ul>

<p>Thank you for choosing <strong>{$brandName}</strong>. Your affordable Dubai stay awaits.</p>
HTML;
    }
...
<strong>Phone:</strong> +971 4 267 3300<br>
<strong>Address:</strong> Al Qusais Industrial Area 1, Dubai, UAE</p>
HTML;
    }
