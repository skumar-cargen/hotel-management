...
class CarAndGeneralSeeder extends Seeder
{
    private int $counter = 200;

    public function run(): void
    {
        $this->command->info('Seeding Car & General Hotels domain...');
        ...
        $this->command->info('Car & General Hotels seeded successfully!');
    }
    ...
    private function seedDomain(): Domain
    {
        $domain = Domain::updateOrCreate(
            ...
            [
                'name' => 'Car & General Hotels',
                'domain' => 'carandgeneralhotels.com',
                'is_active' => true,
                'is_primary' => false,
                'default_currency' => 'AED',
                'default_language' => 'en',
                'meta_title' => 'Car & General Hotels — Luxury Hotels Across the UAE',
                'meta_description' => 'Discover and book luxury hotels across the UAE. From Abu Dhabi to Sharjah and Ras Al Khaimah, experience five-star elegance with stunning views and world-class hospitality.',
                'meta_keywords' => 'car and general hotels, uae luxury hotels, abu dhabi hotels, sharjah hotels, ras al khaimah hotels, dubai hotels, desert resort uae, beach hotel uae',
                'phone' => '+971 4 777 3300',
                'email' => 'info@carandgeneralhotels.com',
    ...
    private function seedHeroSlides(Domain $domain): void
    {
        ...
        $slides = [
            [
                'title' => 'Discover',
                'highlight' => "UAE's Finest Hotels",
                'subtitle' => 'Luxury stays across Abu Dhabi, Dubai, Sharjah & Ras Al Khaimah',
                'description' => 'Explore a curated collection of the finest hotels across the United Arab Emirates.',
                'keywords' => 'uae,luxury,hotel,skyline',
            ],
            [
                'title' => 'Experience',
                'highlight' => 'Coastal Elegance',
                'subtitle' => 'Pristine beaches and world-class waterfront resorts',
                'description' => 'From the Arabian Gulf coast to island retreats, discover beachfront luxury like no other.',
                'keywords' => 'beach,coast,arabian,gulf',
            ],
            [
                'title' => 'Explore',
                'highlight' => 'Desert & City Stays',
                'subtitle' => 'From golden dunes to glittering skylines',
                'description' => 'Choose between serene desert escapes and vibrant city hotels across the UAE.',
                'keywords' => 'desert,resort,city,dubai',
            ],
            [
                'title' => 'Your Gateway To',
                'highlight' => 'Unforgettable Escapes',
                'subtitle' => 'Private beaches, infinity pools & impeccable service',
                'description' => 'Create lasting memories at the UAE\'s most exclusive properties.',
                'keywords' => 'resort,spa,pool,luxury',
            ],
        ];
    ...
    private function seedContentPages(Domain $domain): void
    {
        ...
        $domain->update([
            ...
            'about_us_meta_description' => "Learn about {$brandName} — your gateway to luxury hotels across the UAE. Discover our mission, values, and commitment to hospitality excellence.",
    ...
    private function generateLogoPng(string $path, string $brandName): void
    {
        ...
        // Purple brand color #6d28d9 (RGB: 109, 40, 217)
        $iconColor = imagecolorallocate($img, 109, 40, 217);
        $iconSize = 50;
        imagefilledrectangle($img, 10, 35, 10 + $iconSize, 35 + $iconSize, $iconColor);

        $white = imagecolorallocate($img, 255, 255, 255);
        $initial = 'CG';
        imagestring($img, 5, 20, 52, $initial, $white);

        $textColor = imagecolorallocate($img, 30, 30, 50);
        imagestring($img, 5, 75, 42, 'Car & General', $textColor);
    ...
    private function generateFaviconPng(string $path, string $brandName): void
    {
        ...
        // Purple background #6d28d9 (RGB: 109, 40, 217)
        $bgColor = imagecolorallocate($img, 109, 40, 217);
        imagefilledrectangle($img, 4, 4, $size - 5, $size - 5, $bgColor);

        $white = imagecolorallocate($img, 255, 255, 255);
        $tw = imagefontwidth(5);
        $th = imagefontheight(5);
        imagestring($img, 5, ($size - $tw) / 2, ($size - $th) / 2, 'C', $white);
...
<p>At <strong>{$brandName}</strong>, we connect discerning travellers with the finest luxury hotels across the United Arab Emirates. From the stunning corniche of Abu Dhabi to the golden deserts of Ras Al Khaimah, and from the cultural heart of Sharjah to the glittering skyline of Dubai, we curate a collection of hotels that define excellence in hospitality.</p>

<h3>Our Mission</h3>
<p>Our mission is to make booking a luxury hotel anywhere in the UAE effortless and inspiring. We handpick every property to ensure it meets the highest standards of quality, location, and guest experience — so you can focus on creating unforgettable memories across the Emirates.</p>

<h3>What Sets Us Apart</h3>
<ul>
    <li><strong>UAE-Wide Coverage</strong> — From Abu Dhabi's cultural landmarks to Ras Al Khaimah's desert retreats, we cover the best of every emirate.</li>
    <li><strong>Best Price Guarantee</strong> — We work directly with hotels to negotiate exclusive rates you won't find elsewhere.</li>
    <li><strong>Local Expertise</strong> — Our UAE-based team provides insider knowledge on the best destinations, hidden gems, and seasonal offers across all seven emirates.</li>
    <li><strong>24/7 Concierge</strong> — From airport transfers to desert safari bookings, our dedicated team is available around the clock.</li>
    <li><strong>Flexible Booking</strong> — Free cancellation on most properties up to 48 hours before check-in.</li>
</ul>

<h3>Our Story</h3>
<p>Born from a passion for the UAE's diverse landscapes and world-class hospitality, {$brandName} was founded by a team of travel professionals who saw an opportunity to create a platform showcasing the best hotels across every emirate. We believe that the UAE offers an unparalleled range of experiences — from beachfront luxury and urban sophistication to desert serenity and cultural immersion.</p>
<p>Today we serve thousands of guests each year, helping them discover the perfect UAE getaway — whether it's a palatial stay on Abu Dhabi's Corniche, a family beach holiday in Sharjah, a desert adventure in Ras Al Khaimah, or a city break in Downtown Dubai.</p>
...
<ul>
    ...
    <li><strong>Sustainability</strong> — We partner with eco-conscious hotels and promote responsible tourism across the UAE.</li>
</ul>

<p>Thank you for choosing <strong>{$brandName}</strong>. Your perfect UAE escape awaits.</p>
HTML;
    }
...
<strong>Phone:</strong> +971 4 777 3300<br>
<strong>Address:</strong> Sheikh Zayed Road, Dubai, UAE</p>
HTML;
    }
...
<ul>
    ...
    <li>A tourism dirham fee (AED 7–20 per room per night, depending on property classification) is applied as per UAE Tourism regulations</li>
    <li>Full payment is required at the time of booking unless stated otherwise</li>
    <li>We accept Visa, Mastercard, and American Express through our secure payment gateway</li>
</ul>
